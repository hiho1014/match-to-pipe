...
## [2026-01-20] 중앙 전구 직선 연결 오류 해결 (Straight Wiring Fix)

### 1. 주요 작업 개요
- **목표**: 중앙하단 전구(EXT 3)와 변압기가 가끔 1자로 직선 연결되는 오류 완전 제거.
- **핵심 수정**: `generateRandomPath()` 함수에 방향 우선순위 부여 및 재시도 로직 추가.

### 2. 상세 작업 내역

#### 1) 문제 분석
- **증상**: 변압기와 중앙 전구의 X좌표가 같을 때, 우회 지점을 설정했음에도 불구하고 중앙선을 따라 직선으로 연결되는 경우 발생.
- **원인**: 경로 생성 시 가로/세로 이동을 완전히 랜덤하게 선택하여, 세로 이동이 먼저 선택되면 중앙선을 따라가는 문제.

#### 2) 해결 방안 검토 및 결정
- **검토된 옵션**:
  1. **방향 우선순위 부여** (채택): 우회 시 가로 방향 85% 우선 선택
  2. 직선축 금지 구역 설정 (반려): 경로가 인위적으로 보일 수 있음
  3. 다중 우회 지점 (반려): 배선이 너무 복잡해질 우려
- **의사결정 문서**: `decision_record_20260120_straight_wiring.md` 참조

#### 3) 구현 내용
- **방향 우선순위 (Soft Priority)**:
  - 우회 지점으로 이동 시 가로 방향 이동을 85% 확률로 우선 선택
  - 가로 이동 불가 시 자동으로 세로 이동으로 전환 (유연성 유지)
  
- **재시도 로직 (Retry Loop)**:
  - 경로 생성 실패 시 최대 5회까지 자동 재시도
  - 각 시도마다 다른 랜덤 시드로 경로 생성
  - 사용자는 완성된 결과만 보므로 재시도 과정 인지 불가

- **코드 변경**:
  - `generateRandomPath()`: 재시도 로직 래퍼 함수로 변경
  - `_attemptPathGeneration()`: 실제 경로 생성 로직 (방향 우선순위 적용)

### 3. 테스트 결과
- **테스트 범위**: Stage 1~5 전체
- **결과**: ✅ 모든 스테이지에서 중앙 전구와 변압기 사이의 직선 연결 완전 제거 확인
- **검증 방법**: 브라우저 자동화 테스트 (각 스테이지별 스크린샷 캡처)
...
**관련 문서**: 
- `troubleshooting_20260120_wiring_fix.md` (상세 기술 문서)
- `decision_record_20260120_straight_wiring.md` (의사결정 기록)

---

## [2026-01-20] EXT 번호 체계 개편 및 P-I-P-E 시퀀스 구현

### 1. 주요 작업 개요
- **목표**: 1~3단계의 기존 전구 위치를 유지하면서, EXT 번호를 좌상단부터 순차적으로 매기고, 정답 전구에 'P-I-P-E' 글자 순서를 부여. 4단계부터는 무작위 '꽝(Dud)' 전구 도입.
- **핵심 수정**: `Grid` 클래스의 `endPoints` 생성 로직 및 `addMarker`의 라벨링 로직 변경.

### 2. 상세 작업 내역

#### 1) EXT 번호 및 라벨링 로직 개편 (Sequential Labeling)
- **문제**: 기존 코드에서는 전구의 내부 ID(위치 기반)가 그대로 EXT 라벨로 사용되어, 1단계(전구 2개)에서 "EXT 2, EXT 4"가 뜨는 등 불연속적인 번호가 표시됨.
- **해결**: 
    - 내부적으로는 위치 고정을 위해 기존 ID(4:우하단, 2:좌하단 등)를 유지하되,
    - **UI 표시 라벨(Markup)**은 현재 활성화된 전구 목록의 순서(Index + 1)를 따르도록 변경.
    - 결과적으로 1단계는 항상 "EXT 1, EXT 2", 2단계는 "EXT 1, EXT 2, EXT 3" 순으로 깔끔하게 표시됨.

#### 2) 전구 위치 복원 및 글자 할당 (Position Restoration)
- **위치 정책**: 1~3단계의 전구 등장 순서를 기존 방식(우하단/좌하단 우선)으로 원복.
- **글자 시퀀스**:
    - 활성화된 전구들을 ID(위치) 순으로 정렬.
    - 정렬된 순서대로 'P', 'I', 'P', 'E' 글자를 차례로 할당.
    - 이를 통해 전구 위치가 어디든 상관없이 번호 순서대로 "P-I-P-E" 메시지 전달.

#### 3) 꽝 전구(Dud Bulb) 로직 도입
- **조건**: 스테이지 4 이상(`stageConfig.id >= 4`)에서 발동.
- **동작**: 
    - 5개의 전구 중 무작위 1개를 선정하여 글자(`secret`)를 제거.
    - 불은 정상적으로 켜지지만 글자가 나타나지 않는 '꽝' 역할 수행.
    - 나머지 4개 전구에만 P-I-P-E 글자 할당.

### 3. 사용자 피드백 반영
- 초기 수정 시 전구 위치를 강제로 변경하여 발생한 사용자 불만을 수용, **"위치는 그대로, 번호와 규칙만 변경"**하는 절충안을 신속히 적용하여 만족도 확보.
